...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort" id="layer-photos">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><?php echo $cates[$vo['cate_id']];?></td>
				<td><?php echo delimiter($vo['params']);?></td>
				<td><?php echo img_style($vo['img']);?></td>
				<td><?php echo ($vo['introduction']); ?></td>
				<td><?php echo $vo['status']==1?'显示':'关闭';?></td>
				<td><?php echo $vo['update_time']?date('Y-m-d H:i:s',$vo['update_time']):'';?></td>
...
function status_switch(obj,id,title){
    ...
    layer.confirm('确认要'+content+title+'吗？',function(index){
        $.get("<?php echo U('goodsSwitchStatus');?>",{id:id},function(result){
            layer.msg(result['msg']);
            if(!result['code']){
                $(obj).text(str);

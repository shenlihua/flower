...
class WechatController extends Controller{
    ...
    public function index($id = ''){
        ...
        try{
            /*测试帐号*/
            $appid = 'wx7502c44234f7f6d8'; //AppID(应用ID)
            $token = 'asuan'; //微信后台填写的TOKEN
            $crypt = 'q6FPCUoCQWaOiR3UUe5RfQu8A7hlJcMW4BnNyH9z2il'; //消息加密KEY（EncodingAESKey）

            /*订阅号*/
//            $appid = 'wx6cb25dfd791d6bff'; //AppID(应用ID)
//            $token = 'asuan'; //微信后台填写的TOKEN
//            $crypt = 'q8NoP22fzaZ0pz0s7p3YNNupfI1lTObs2OWOwPVkVGS'; //消息加密KEY（EncodingAESKey）
    ...
    public function createMenu(){
        ...
        $data=$auth->menuGet();
        $arr=array("type"=>'view',"name"=>"千艺花卉","url"=>"http://slh.tunnel.phpor.me/qyhf/wechat.php");
        array_push($data['menu']['button'],$arr);
        $return=$auth->menuCreate($data['menu']['button']);
        dump($return);
    }
    /**
     * DEMO
